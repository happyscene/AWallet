    ...
    ui(new Ui::MainPage)
{
	...
	connect(DataMgr::getInstance(), &DataMgr::onWalletAccountCreate, this, &MainPage::walletAccountCreate, Qt::UniqueConnection);
    ui->label->hide();

	Frame* frame = dynamic_cast<Frame*>(Goopal::getInstance()->mainFrame);
	connect(frame, &Frame::updateTotalBalance, this, &MainPage::updateTotalBalance);
...
void MainPage::updateAccountList()
{
	...
	for( int i = 0; i < size; i++) {
        ...
        QTableWidgetItem* item1= ui->accountTableWidget->item(rowNum, 1);
		...
		if (currencyInfo.isAsset()) {
			...
			double dValue = balance.toDouble();
...
void MainPage::updateTotalBalance()
{
	...
	if (currencyInfo.isAsset()) {
        foreach (CommonAccountInfo account , *(DataMgr::getInstance()->getAccountInfo())){
			QString balance = account.getBalance(currencyInfo.name);
            totalBalance += balance.toDouble();
        }
	} else {
        foreach (CommonAccountInfo account , *(DataMgr::getInstance()->getAccountInfo())){
			QString balance = DataMgr::getInstance()->getAccountTokenBalance(account.name, currencyInfo.name);
            totalBalance += balance.toDouble();
        }
	}

    ui->totalBalanceLabel->setText("<body><font style=\"font-size:20px\" color=#000000>" + checkZero(totalBalance)
                                   + "</font><font style=\"font-size:12px\" color=#000000>"
								   + " " + DataMgr::getCurrCurrencyName() + "</font></body>");
	int height = ui->totalBalanceLabel->geometry().height();
	ui->totalBalanceLabel->adjustSize();
	QRect labelGeometry = ui->totalBalanceLabel->geometry();
	labelGeometry.setHeight(height);
	ui->totalBalanceLabel->setGeometry(labelGeometry);

    ui->totalBalanceLabel->move(555, 44);
    ui->label->move(553 - ui->label->width(), 42);
