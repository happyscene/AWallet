...
struct CommonAccountInfo {
	...
	QString getBalance(const QString& asset) const
	{
		QMap<QString, QString>::const_iterator itr = balances.find(asset);
		if (itr != balances.end())
		{
			return itr.value();
		}
...
struct LedgerEntries {
	...
	QString getBalance(QString account, QString asset_type) const
	{
		...
		if (itr != running_balances.end())
		{
			...
			if (_itr != (*itr).end())
				return doubleToStr(_itr->amount);
		}
...
public:
	...
	QString getAccountTokenBalance(QString account, QString asset);
	...
	void walletGetInfo();
	void walletClose();
	void walletOpen(const QString& name);
	...
	void walletCheckPassphrase(const QString& password);
	...
	void tokenQueryBalance(QString contractId, QString call_name);
	void tokenTransferTo(QString call_name, QString contractId, QString to_address, double amount, double fee, QString remark);
...
public:
	...
	CurrencyInfo getCurrencyByConId(const QString& contractId);
	...
	void deleteAccountInfo(const QString& account);
	QMap<QString, QMap<QString, TokenAccountInfo>>* getTokenAccountInfo();
